...
export default function DriverBookingDetailPage() {
  ...
  async function updateStatus(newStatus: string) {
    ...
    if (error) {
      ...
      fetch(`${ADMIN_URL}/api/notify/driver-action`, {
        method: 'POST',
  ...
  return (
    <div>
      ...
      <div className="bg-white border-b border-gray-100 px-5 py-3 flex items-center justify-between">
        <Link href="/" className="text-blue-700 hover:text-blue-900 text-sm font-medium transition-colors">
          ← Mes courses
        </Link>
        ...
        {booking.status === 'cancellation_requested' && (
          <div className="bg-amber-50 border border-amber-300 rounded-xl p-4 text-center">
            <p className="text-sm text-amber-800 font-medium flex items-center justify-center gap-2">
              <Clock className="w-4 h-4" /> Demande d'annulation en attente de décision admin
            </p>
          </div>
        )}

        {['completed', 'cancelled', 'no_show', 'refused'].includes(booking.status) && (
          <Link href="/" className="block text-center text-blue-700 text-sm font-medium py-3">
