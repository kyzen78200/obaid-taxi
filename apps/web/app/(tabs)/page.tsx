...
export default function BookingPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (forfaitMode) {
      ...
      const matchingZones = zones.filter(z =>
        haversineDistance({ lat: clientCoords.lat, lng: clientCoords.lng }, { lat: z.center_lat, lng: z.center_lng }) <= z.radius_km
      )
      ...
      const fallbackKm = async () => {
        ...
        const estimate = calculateFare({ distanceKm: routeInfo.distance_km, durationMin: routeInfo.duration_min, departureTime: new Date(scheduledAt), tripType: 'one_way' })
        saveBookingSession({
      ...
      if (!closestZone) { await fallbackKm(); return }

      const { data: pkg } = await supabase
        .from('fare_packages').select('id, price')
        .eq('zone_id', closestZone.id).eq('name', selectedDest.name).eq('active', true).single()

      if (!pkg) { await fallbackKm(); return }

      const forfaitEstimate = calculateFare({
        distanceKm: 0, durationMin: 0,
        departureTime: new Date(scheduledAt), tripType: 'one_way',
        forfaitPrice: pkg.price, forfaitName: selectedDest.name,
      })
      saveBookingSession({
        pickup_address: pickupAddr, pickup_lat: originCoords.lat, pickup_lng: originCoords.lng,
        dropoff_address: dropoffAddr, dropoff_lat: dropoffCoordsFinal.lat, dropoff_lng: dropoffCoordsFinal.lng,
    ...
    if (!routeInfo) { setError("Impossible de calculer l'itinéraire. Vérifiez les adresses."); setLoading(false); return }
    const estimate = calculateFare({ distanceKm: routeInfo.distance_km, durationMin: routeInfo.duration_min, departureTime: new Date(scheduledAt), tripType })
    saveBookingSession({
      ...
      scheduled_at: scheduledAt, trip_type: tripType, is_conventional: isConventional,
      distance_km: routeInfo.distance_km, duration_min: routeInfo.duration_min,
      tariff_code: estimate.tariff_code, base_price: estimate.base_price,
      estimated_min: estimate.estimated_min, estimated_max: estimate.estimated_max,
      forfait_id: null, forfait_name: null,
  ...
  return (
    <div className="bg-gray-50">
      ...
      <div className="max-w-lg mx-auto px-4 -mt-6 pb-6">
        <div className="bg-white rounded-2xl shadow-lg p-6">
          <form onSubmit={handleSubmit} className="space-y-4">

            {/* Toggle forfait */}
            <div className="flex gap-2">
              <button type="button" onClick={() => { setForfaitMode(false); setSelectedDest(null) }}
              ...
              <button type="button" onClick={() => setForfaitMode(true)}
                className={`flex-1 py-2 rounded-xl text-sm font-medium transition-colors border ${forfaitMode ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-gray-300 text-gray-700 hover:border-blue-400'}`}>
                🚉 Gare / Aéroport
              </button>
            </div>
            ...
            <div>
              ...
              <input ref={pickupRef} type="text" value={pickup}
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {([{ value: 'to', label: 'Aller vers →' }, { value: 'from', label: '← Venir de' }] as const).map(opt => (
                    ...
                    <div className="space-y-2">
                      {gares.length > 0 && (
                        <div>
                          ...
                          {gares.map(dest => (
                            <button key={dest.name} type="button" onClick={() => setSelectedDest(dest)}
            ...
            {/* Type (masqué en forfait) */}
            {!forfaitMode && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Type de course</label>
                <div className="grid grid-cols-2 gap-2">
                  {([{ value: 'one_way', label: 'Aller simple' }, { value: 'round_trip', label: 'Aller-retour' }] as const).map(opt => (
            ...
            <label className="flex items-center gap-3 py-2 cursor-pointer select-none">
              <input type="checkbox" checked={isConventional} onChange={e => setIsConventional(e.target.checked)}
            ...
            {isConventional && (
              <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
                <p className="text-xs text-blue-700">Vous pourrez joindre votre attestation de transport (PDF) à l'étape suivante.</p>
              </div>
            )}
